...
W2 = @(F,a,t) -(1/2)*K*((1/3)*(a.^2).*((T-t).^2).*N2(d2(F,a,T-t,K))...
    + (1/3)*((T-t).^(3/2)).*d2(F,a,T-t,K).*N2(d2(F,a,T-t,K))...
    -(1/6)*((T-t).^(3/2)).*N1(d2(F,a,T-t,K)))...
    +(1/2)*rho*rho*K*((1/4)*(a.^2).*((T-t).^2).*N4(d2(F,a,T-t,K))...
...
source3 = (1/6)*rho*K*(T-t)*ag.*N2(d2(Fg,ag,T-t,K))...
    -(1/12)*rho*K*(T-t)^(1/2)*ag.*(-4*(T-t)*ag+sqrt(T-t).*d2(Fg,ag,T-t,K)).*N3(d2(Fg,ag,T-t,K))...
    -rho*K*ag.*((5/12)*((T-t)*ag+sqrt(T-t).*d2(Fg,ag,T-t,K))+(1/8)*rho*rho*(T-t)).*N4(d2(Fg,ag,T-t,K))...
    -(1/8)*rho*K*(T-t)^(3/2)*(ag.^2).*N5(d2(Fg,ag,T-t,K))...
    +(1/8)*rho^3 * K * ag .*((T-t)*ag+sqrt(T-t).*d2(Fg,ag,T-t,K)).*N6(d2(Fg,ag,T-t,K));
